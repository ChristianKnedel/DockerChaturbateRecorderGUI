class DockerAdapter(object):
   ...
   def startInstance(self, media_path, containerName, title, limit_in_gb, imageName):
        limit_in_kb = int( int( limit_in_gb ) * 1024 * 1024)
        return "docker run -d -e LIMIT_MAXIMUM_FOLDER_KB={} --rm -v {}:/output --name {} {} /code/recorder.sh -u https://chaturbate.com/{}/ &".format(
            limit_in_kb,
            media_path, 
            containerName, 
            imageName,
